...
class FH {
  ...
  public static function passwordBlock($type, $label, $name, $value='', $inputAttrs=[],$passWordToggleAttrs=[], $passwordContainerAttrs=[], $divAttrs=[],$errors=[]){
    $inputAttrs = self::appendErrorClass($inputAttrs,$errors,$name,'is-invalid');
    $divString = self::stringifyAttrs($divAttrs);
    $passwordContainerString = self::stringifyAttrs($passwordContainerAttrs);
    $inputString = self::stringifyAttrs($inputAttrs);
    $passWordToggleString = self::stringifyAttrs($passWordToggleAttrs);
    $id = str_replace('[]','',$name);
    $html = '<div' . $divString . '>';
    $html .= '<label class="form-label" for="'.$id.'">'.$label.'</label>';
    $html .= '<div' . $passwordContainerString . '>';
    $html .= '<input type="'.$type.'" id="'.$id.'" name="'.$name.'" value="'.$value.'"'.$inputString.' />';
    $html .= '<button'  . $passWordToggleString . '>';
    $html .= ' 👁️ ';
    $html .= '</button>';
    $html .= '</div>';
    $html .= '</div>';
  ...
  public static function checkboxBlock($label,$name,$checked=false,$inputAttrs=[],$divAttrs=[],$errors=[]){
    ...
    $id = str_replace('[]','',$name);
    $html = '<div'.$divString.'>';
    $html .= '<label for="'.$id.'" class="form-label">'.$label.' <input type="checkbox" id="'.$id.'" name="'.$name.'" value="on"'.$checkString.$inputString.'>';
    $html .= '<span class="invalid-feedback">'.self::errorMsg($errors,$name).'</span>';
    $html .= '</label>';
    $html .= '</div>';
    return $html;
  ...
  public static function checkboxBlock2($label,$name,$checked=false,$inputAttrs=[],$divAttrs=[],$errors=[]){
    $inputAttrs = self::appendErrorClass($inputAttrs,$errors,$name,'is-invalid');
    $divString = self::stringifyAttrs($divAttrs);
    $inputString = self::stringifyAttrs($inputAttrs);
    $checkString = ($checked)? ' checked="checked"' : '';
    $id = str_replace('[]','',$name);
    $html = '<div'.$divString.'>';
    $html .= '<input type="checkbox" id="'.$id.'" name="'.$name.'" value="on"'.$checkString.$inputString.'>';
    $html .= '<label  class="form-label" for="'.$id.'">'.$label.'</label>';
    $html .= '<span class="invalid-feedback">'.self::errorMsg($errors,$name).'</span>';
    $html .= '</div>';
    return $html;
